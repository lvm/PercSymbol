/*
        ...
        (c) 2017, 2018 by Mauro <mauro@sdf.org>
...
PercSymbol {
  ...
  *directory {
  ...
  prMidiNote { |input = \rest|
    // recursive funkyness
    if(input.isKindOf(Collection)) { ^input.collect(this.prMidiNote(_)); };
  ...
  *doesNotUnderstand { |selector, args|
		var midinote = this.at(selector, args).deepCopy;
		^midinote ?? { super.doesNotUnderstand(selector, args) };
	}

  printOn { |stream| this.storeOn(stream) }
  storeOn { |stream| stream << "PercSymbol(\\" << symbol << ")"; }
  storeArgs { ^[symbol] }
...
+ String {
  asPercSymbol { ^PercSymbol(this.asSymbol); }
}
